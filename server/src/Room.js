...
class Room extends EventEmitter {
    constructor(server, _id, settings) {
        ...
        this.chatmsgs = JSON.parse(fs.readFileSync(this.chatmsgsfile, 'utf8', function(err,data){return })) || [];
        this.ppl = new Map();
        this.connections = [];
    ...
    join(cl) { //this stuff is complicated
        ...
        if (!otheruser) {
            ...
            if (((Array.from(this.ppl.values()).length == 0) && !this.isLobby(this._id)) || this.crown && (this.crown.userId == cl.user._id)) { //user that created the room, give them the crown.
    ...
    chat(p, msg) {
        ...
        var rulemessage = [{m:"a", message:p.user.name+", your message was found to contain rule-breaking content and was blocked. Please read the rules here https://docs.google.com/document/d/1wQvGwQdaI8PuEjSWxKDDThVIoAlCYIxQOyfyi4o6HcM/ Attempting to bypass this filter may result in a ban.", p:{
            name: "server",
            color: p.user.color,
            id: p.participantId,
            _id: p.user._id,
        ...
        if (regexp.test(msg.message)) return this.sendArray(rulemessage);
        ...
        if (prsn) {
            ...
            message.p = {
                ...
                _id: p.user._id,
                tag: p.user.tag,
                tagcolor: p.user.tagcolor
            };
            message.t = Date.now();
            this.sendArray([message]);
            this.chatmsgs.push(message);
            fs.writeFileSync(this.chatmsgsfile, JSON.stringify(this.chatmsgs, true, 2), function(err){if (err)console.log(err);})
        }
    }
    playNote(cl, note) {
        this.sendArray([{
