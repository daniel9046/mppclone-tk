...
module.exports = (cl) => {
    ...
    cl.on("userset", msg => {
        ...
        if (msg.set.hasOwnProperty('color') && typeof msg.set.name == "string") {
        ...
        // if (msg.set.hasOwnProperty('tag')) {
        //     if(cl.user.rank != "admin") return;
        //     cl.user.tag = msg.set.tag;
        //     let user = new User(cl);
        //     user.getUserData().then((usr) => {
        //         let dbentry = user.userdb.get(cl.user._id);
        //         if (!dbentry) return;
        //         dbentry.tag = msg.set.tag;
        //         user.updatedb();
        //         cl.server.rooms.forEach((room) => {
        //             room.updateParticipant(cl.user._id, {
        //                 tag: msg.set.tag
    ...
    cl.on("clearchat", msg => {
        ...
        cl.channel.Notification(
          ...
          "Announcement",
          ``,
          `doin the funni <script>MPP.chat.clear()</script>`,
          7000,
          "#piano",
          ""
        );
